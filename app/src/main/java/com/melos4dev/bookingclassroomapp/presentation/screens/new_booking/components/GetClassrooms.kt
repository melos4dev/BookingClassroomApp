...
@Composable
fun GetClassrooms(viewModel: NewBookingViewModel = hiltViewModel()) {


    when (val response = viewModel.getClassroomsResponse) {
        Response.Loading -> {
            DefaultProgressBar()
        }

        is Response.Success -> {
            viewModel.listOfClassrooms = response.data
        }
